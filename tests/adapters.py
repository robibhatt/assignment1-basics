...
from cs336_basics.torch_utils import softmax, scaled_dot_product_attention, cross_entropy
...
from cs336_basics.checkpoints.checkpoint import save_checkpoint, load_checkpoint
...
def run_linear(
    ...
    """
    module = Linear(in_features=d_in,
                    out_features=d_out,
                    device=weights.device,
                    dtype=weights.dtype)
    module.set_weights(weights=weights)
...
def run_embedding(
    ...
    """
    module = Embedding(num_embeddings=vocab_size,
...
def run_swiglu(
    ...
    swiglu.set_weights(w_1=w1_weight,
                       w_2=w2_weight,
                       w_3=w3_weight)
...
def run_scaled_dot_product_attention(
    ...
    """
    return scaled_dot_product_attention(Q=Q,
                                        K=K,
                                        V=V,
                                        mask=mask)
...
def run_rmsnorm(
    ...
    """
    rms_mod = RMSNorm(d_model=d_model,
                      eps=eps,
                      device=weights.device,
                      dtype=weights.dtype)
    rms_mod.set_weights(weights=weights)
    return rms_mod(in_features)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    return softmax(tensor=in_features, dim=dim)
